 ...
 * constraints, and the same Attachment, ParameterDefinition, DataRequirement,
 * and RelatedArtifact structures. Their FML maps have the same omissions, so
 * this pair reuses the R4 <-> R5 logic. Only descriptor names and descriptions
 * differ so reports identify the actual conversion hop.
 *
...
export const conv_R4B_to_R5 = {
  ...
  description:
    ...
    + 'normalizes or removes ParameterDefinition and DataRequirement FHIR types that '
    + 'changed across the hop. Reuses the R4->R5 transform. Does not handle inter-version '
    + 'extensions.',
};
...
export const conv_R5_to_R4B = {
  ...
  description:
    ...
    + 'RelatedArtifact content, normalizes or removes version-specific ParameterDefinition '
    + 'and DataRequirement FHIR types, and drops related artifacts whose R5-only '
    + 'relationship type is invalid in R4B. Reuses the R5->R4 transform. Does not handle '
